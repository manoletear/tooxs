...
import logoMaestra from "../assets/logos/maestra.png";
import logoTepille from "../assets/logos/tepille.png";
import logoReutter from "../assets/logos/reutter.png";
import logoLegalChile from "../assets/logos/legalchile.png";
import logoMatrix from "../assets/logos/matrix.png";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:url", content: "https://tooxs.lovable.app" },
    ...
    links: [
      { rel: "canonical", href: "https://tooxs.lovable.app" },
    ],
  }),
  component: Index,
});
          ...
          <div className={`lg:max-w-[32%] transition-all duration-1000 delay-300 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-2.5">
                <button
                  onClick={() => openMeeting("Hero — Inicio")}
                  className="inline-flex items-center gap-2 bg-white text-navy px-5 py-2.5 rounded-full font-semibold hover:bg-white/95 transition-all duration-300 text-xs group shadow-lg"
                >
                  Reunión
                  <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-primary text-white group-hover:scale-110 transition-transform">
                    <ArrowRight size={11} />
                  </span>
                </button>
                <Link
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-8 items-center">
          ...
          <ScrollReveal delay={200} className="flex flex-col items-center h-full">
            <div className="bg-card rounded-xl p-4 shadow-sm border border-border/50 w-full h-full flex items-stretch justify-center gap-3">
              ...
              />
              <ul
                className="flex flex-col justify-center gap-2 pl-1 border-l border-border/50"
                aria-label="Países con presencia regional"
              >
                {["Chile", "Perú", "Colombia", "Costa Rica", "Panamá"].map((country) => (
                  <li
                    key={country}
                    className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-navy whitespace-nowrap"
                  >
